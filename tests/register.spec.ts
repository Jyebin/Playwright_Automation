...
test.describe('T759 - 소셜 회원가입 버튼 확인', () => {
  ...
  // ── 자동화 불가 항목 (소셜 로그인 완료) ───────────────────────────────────
  test('카카오 계정으로 실제 로그인 완료 후 회원가입 처리', { annotation: tcStep(1) }, async () => {
    test.skip(true, '카카오 로그인은 외부 OAuth 서비스(실제 카카오 계정 로그인·봇 감지) — 자동화 대상 아님 (수동 확인)');
  });

  test('구글 계정으로 실제 로그인 완료 후 회원가입 처리', { annotation: tcStep(2) }, async () => {
    test.skip(true, '구글 로그인은 외부 OAuth 서비스(실제 구글 계정 로그인·2단계 인증) — 자동화 대상 아님 (수동 확인)');
  });

  test('네이버 계정으로 실제 로그인 완료 후 회원가입 처리', { annotation: tcStep(3) }, async () => {
    test.skip(true, '네이버 로그인은 외부 OAuth 서비스(실제 네이버 계정 로그인·2단계 인증) — 자동화 대상 아님 (수동 확인)');
  });
});
...
//    Step 3(약관 모달)은 tests/register-terms.spec.ts 에서 메일함의 기존 인증 링크로 검증 (이 파일은 serial 이라 분리).
// ─────────────────────────────────────────────────────────────────────────────
test.describe('T758 - 이용약관 동의', () => {
  ...
  test('비밀번호 입력 + 필수 약관 동의 후 [회원가입] 클릭 → 회원가입 완료 페이지 이동', { annotation: tcStep(6) }, async ({ page }) => {
    // ⚠️ 실제 계정이 생성되는 테스트: reCAPTCHA 가 통과되는 경우 앞 테스트까지 통과하면 실제 가입이 진행됨
    //    → 명시적으로 허용했을 때만 실행 (ALLOW_SIGNUP=1)
    test.skip(process.env.ALLOW_SIGNUP !== '1', '실제 회원가입(계정 생성)이 진행되는 테스트 — ALLOW_SIGNUP=1 일 때만 실행');
    const register = new RegisterPage(page);
    await register.fillPassword('TestPass1!', 'TestPass1!');
    await register.checkAllTermsAgree();
    await register.clickRegisterButton();
