...
import { tcCase, tcStep } from './utils/evidence';
...
test('아이디에 한글 입력 시 유효성 메세지', { annotation: [tcCase('T760'), tcStep(2)] }, async ({ page }) => {
  const loginPage = new LoginPage(page);
...
test('아이디를 숫자로 시작 시 유효성 메세지', { annotation: [tcCase('T760'), tcStep(2)] }, async ({ page }) => {
  const loginPage = new LoginPage(page);
...
test('아이디 최소 입력 글자 수 미달 시 유효성 메세지', { annotation: [tcCase('T760'), tcStep(2)] }, async ({ page }) => {
  const loginPage = new LoginPage(page);
...
test('아이디 최대 입력 글자 수 초과 시 254자로 제한', { annotation: [tcCase('T760'), tcStep(2)] }, async ({ page }) => {
  const loginPage = new LoginPage(page);
...
test('비밀번호 최대 입력 글자 수 초과 시 20자로 제한', { annotation: [tcCase('T760'), tcStep(3)] }, async ({ page }) => {
  const loginPage = new LoginPage(page);
...
test('비밀번호 미입력 상태로 로그인 버튼 클릭 시 모달', { annotation: [tcCase('T760'), tcStep(2)] }, async ({ page }) => {
  const loginPage = new LoginPage(page);
