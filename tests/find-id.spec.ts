...
test.describe('T419 아이디 찾기 - 휴대폰 번호로 찾기', () => {
  test('아이디 찾기 페이지 이동 및 휴대폰 번호로 찾기 탭 선택', { annotation: tcStep(1) }, async ({ page }) => {
  ...
  test('이름 / 휴대폰 번호 / 인증번호 placeholder 확인', { annotation: tcStep(2) }, async ({ page }) => {
    const findIdPage = new FindIdPage(page);
  ...
  test('이름 + 휴대폰 번호 입력 시 인증번호 받기 버튼 활성화 확인', { annotation: tcStep(3) }, async ({ page }) => {
    const findIdPage = new FindIdPage(page);
  ...
  test('잘못된 이름/휴대폰 번호 입력 → "가입 시 등록한 정보를 다시 확인해 주세요." 알럿', { annotation: tcStep(4) }, async ({ page }) => {
    const findIdPage = new FindIdPage(page);
  ...
  test('취소 버튼 클릭 → 로그인 페이지 이동', { annotation: tcStep(13) }, async ({ page }) => {
    const findIdPage = new FindIdPage(page);
  ...
  test('인증번호 발송 및 인증 완료 → 아이디 찾기 결과 확인', { annotation: tcStep(5, 8) }, async ({ page }) => {
    test.skip(true, '실제 SMS 인증 필요 — 자동화 제외 (인증번호 발송/입력/결과 확인)');
  });
  test('인증 5회 실패 → 제한 초과 알럿 확인', { annotation: tcStep(6) }, async ({ page }) => {
    test.skip(true, '실제 SMS 인증 필요 — 자동화 제외');
  });
  test('인증번호 입력 시간 만료 → 만료 알럿 확인', { annotation: tcStep(7) }, async ({ page }) => {
    test.skip(true, '실제 SMS 인증 필요 — 자동화 제외 (3분 만료 대기)');
  });
  test('인증 잔여 횟수 초과 → 24시간 제한 알럿', { annotation: tcStep(12) }, async ({ page }) => {
...
test.describe('T423 아이디 찾기 - 이메일 주소로 찾기', () => {
  test('아이디 찾기 페이지 이동 및 이메일 주소로 찾기 탭 선택', { annotation: tcStep(1) }, async ({ page }) => {
    const findIdPage = new FindIdPage(page);
    await test.step('[셋업] 아이디 찾기 페이지 이동', async () => {
      await findIdPage.navigate();
    });
  ...
  test('이름 / 이메일 주소 / 인증번호 placeholder 확인', { annotation: tcStep(2) }, async ({ page }) => {
    const findIdPage = new FindIdPage(page);
  ...
  test('이름 + 이메일 입력 시 인증번호 받기 버튼 활성화 확인', { annotation: tcStep(3) }, async ({ page }) => {
    const findIdPage = new FindIdPage(page);
  ...
  test('잘못된 이름/이메일 입력 → "가입 시 등록한 정보를 다시 확인해 주세요." 알럿', { annotation: tcStep(4) }, async ({ page }) => {
    const findIdPage = new FindIdPage(page);
    await test.step('[셋업] 잘못된 정보 입력 및 인증번호 받기 클릭', async () => {
      await findIdPage.navigate();
      await findIdPage.selectEmailTab();
  ...
  test('취소 버튼 클릭 → 로그인 페이지 이동', { annotation: tcStep(14) }, async ({ page }) => {
    const findIdPage = new FindIdPage(page);
    await test.step('[셋업] 아이디 찾기 페이지 이동 및 취소 클릭', async () => {
      await findIdPage.navigate();
      await findIdPage.clickCancel();
  ...
  test('인증번호 발송 및 인증 완료 → 아이디 찾기 결과 확인', { annotation: tcStep(5, 9) }, async ({ page }) => {
    test.skip(true, '실제 이메일 인증 필요 — 자동화 제외');
  });
  test('인증 5회 실패 → 제한 초과 알럿 확인', { annotation: tcStep(7) }, async ({ page }) => {
    test.skip(true, '실제 이메일 인증 필요 — 자동화 제외');
  });
  test('인증번호 입력 시간 만료 → 만료 알럿 확인', { annotation: tcStep(8) }, async ({ page }) => {
    test.skip(true, '실제 이메일 인증 필요 — 자동화 제외');
  });
  test('인증 잔여 횟수 초과 → 24시간 제한 알럿', { annotation: tcStep(13) }, async ({ page }) => {
...
test.describe('T510 비밀번호 찾기 - 비밀번호 재설정', () => {
  test('비밀번호 재설정 링크를 통한 재설정 플로우', { annotation: tcStep(1) }, async ({ page }) => {
